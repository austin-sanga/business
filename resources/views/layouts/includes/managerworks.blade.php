<div class="row ">
    <div class="col ms-5">
        <a href="investments" class="text-decoration-none">
        <div class="card bg-white mx-3">
        <div><i class="bi bi-currency-dollar" style="font-size:40px; color:#0B9050"></i></div>
        <div><span style="color: black">Investments</span>
        </div>
        </div>
        </a>
    </div>

    <div class="col me-5">
        <a href="loans" class="text-decoration-none">
        <div class="card bg-white mx-3">
        <div><i class="bi bi-cash-coin" style="font-size:40px; color:#0B9050"></i></div>
        <div><span style="color: black">Loans</span>
        </div>
        </div>
        </a>
    </div>
</div>
<div class="row mt-2">
    <div class="col ms-5">
        <a href="loans" class="text-decoration-none">
        <div class="card bg-white mx-3">
        <div><i class="bi bi-person-fill" style="font-size:40px; color:#0B9050"></i></div>
        <div><span style="color: black">Manager</span>
        </div>
        </div>
        </a>
    </div>
    <div class="col me-5">
        {{-- this cheats around and make it seem there is a column beside the top --}}
        {{-- <a href="investments" class="text-decoration-none">
        <div class="card bg-white mx-3">
        <div><i class="bi bi-people-fill" style="font-size:40px; color:#0B9050"></i></div>
        <div><span style="color: black">Membership</span>
        </div>
        </div>
        </a> --}}
    </div>
</div>
